...
- `SetLog` — one logged set (`setNumber`, `repsDone`, `weightKg`, `loggedAt`); no cap on rows returned per exercise (`findAllByWorkoutExercise` returns the full history, ordered ascending).
...
The catalog holds roughly 170 exercises, mostly defaulted to `INTERMEDIARIO` difficulty with a small hand-curated `isFeatured` subset per muscle group. EN/ES translations exist as an incremental, possibly-partial overlay (fallback to PT is normal, not an error state). The workout generator and post-workout summary (volume delta, PR badges, streak) are both live and wired into the normal create/complete flows. Student-trainer relations support payment reminders fired at login (no scheduler/cron infra). `SetLog` history has no pagination or row cap yet — history endpoints return everything for a given exercise.
