...
- Downgrade (`applyFreePlan`) never removes existing `ClientRelation` rows past the new limit — the rows survive. **But this is no longer "no consequences going forward": Fase 103 added a retroactive cutoff on top.** `src/lib/plan-expiry.ts` tracks `overLimiteAlunosSince` and, after `PERSONAL_OVER_LIMIT_GRACE_DAYS = 5`, `assertPersonalCanPrescribe` blocks the professional from prescribing (403 `PERSONAL_OVER_LIMIT`) and `assertAlunoWorkoutAccessible` blocks the *student* from opening those workouts (403 `PERSONAL_PLAN_RESTRICTED`) — wired into 8 call sites across `src/fitness/`. So over-limit professionals DO lose write access, and already-linked students ARE affected, after the grace period. Treat those 403s as intended behavior, not bugs. (This bullet previously forbade exactly what Fase 103 shipped — corrected in the 2026-08-06 audit.) The schema columns backing it are `planoAssinaturaExpiresAt` and `overLimiteAlunosSince`.
...
Three tiers exist and are live in code: `FREE` (3 students, no directory access), `BASE` (20 students, can opt into the professional directory), `PLUS` (effectively unlimited students, priority placement in the directory — priority ordering itself lives outside this domain). Checkout supports **monthly and quarterly** intervals for both paid tiers (4 Stripe Price IDs total: `STRIPE_PRICE_ID_{BASE,PLUS}_{MONTHLY,QUARTERLY}`). There is **no annual interval** — `BillingInterval` is `"monthly" | "quarterly"` (`stripe.ts`), and `billing.controller.ts` coerces anything that isn't `"quarterly"` into `"monthly"`, so building an "annual" option would silently charge one month while selling a year. (This line said "annual" until the 2026-08-06 audit; `.env.example` still had `_ANNUAL` vars the code never reads — also corrected.) Real Stripe products/prices and a live webhook endpoint are not yet configured in any environment (test-mode only, per `BILLING_SETUP.md`) — going live is a manual, explicit step (swap test keys for live keys and deploy), not a code change.
